...
crawl() {
mac11="$(ip addr show dev br0 | grep ether | cut -d " " -f 6)"
...
#    . /etc/firmware_release
    ...
    SYSTEM_DATA=$SYSTEM_DATA"<firmware_version>$(cat /etc/version)</firmware_version>"
...
tmp=$(mktemp)
echo "$send" >> $tmp
curl -v -H "Content-type: application/json; charset=UTF-8" $HEADER --data-binary @$tmp https://monitoring.freifunk-franken.de/api/alfred
echo "Data send complete"
rm "$tmp"
